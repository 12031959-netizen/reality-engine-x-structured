...
const STORAGE_KEY = "reality-engine-x-feedback";
...
function saveFeedback(items) {
  try {
    ...
    // Feedback still works for the current session if storage is unavailable.
  }
}
...
export default function Feedback() {
  ...
  async function handleSubmit(event) {
    ...
    setItems(nextFeedback);
    saveFeedback(nextFeedback);

    try {
      await apiClient.post(`/accounts/${user.id}/feedback`, feedback);
    } catch {
      // Feedback remains saved locally if the API server is not running.
    }
